...
class Selector(object):
    ...
    @staticmethod
    def _get_importance_features(model,columns,topn=48,ascending=False):
    ...
    def _get_cv_error(self,x_train,y_train):
        ...
        model_list = [self._get_lgb_model()]   #xgb速度过慢，只用lgb尝试
    ...
    def _get_increase_features(self,x_train,y_train):
        ...
        for model in [self._get_lgb_model()]:
...
if __name__ == '__main__':
    dataset = pd.read_csv(r'F:\DataFountain\feature_dataset\features_lgb_2.csv')
    ...
    select_features = Selector(ascending=True).get_select_features(x_data,y_data)
    select_features.extend(['id','score'])
    dataset  = dataset[select_features]
    dataset.to_csv(r'F:\DataFountain\feature_dataset\features_lgb_3.csv',index=False,encoding='utf-8')
