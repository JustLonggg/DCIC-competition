...
class Processing(object):
    ...
    def _get_data(self):
        ...
        train_data_name = os.path.join(TRAIN_PATH,'train_dataset.csv')
        train_data = pd.read_csv(train_data_name,header=0)
        train_data.columns = columns_name
        
        test_data_name = os.path.join(TEST_PATH,'test_dataset.csv')
    ...
    def _get_app_rate(dataset):
        ...
        dataset['helper_sum'] = dataset[app_num_columns].apply(lambda x:np.log1p(np.sum(x)),axis=1)
